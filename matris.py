...
class Matris(object):
    def __init__(self): #OK
        ...
        self.combo = 1 # 一開始的COMBO數
        
        self.paused = False

        self.highscore = load_score()  #讀取分數最高紀錄
    ...
    def update(self, timepassed): #OK
        ...
        for event in events:
            ...
            if pressed(pygame.K_SPACE):    #按下空白鍵，下降到底
                self.hard_drop()
            elif pressed(pygame.K_UP) or pressed(pygame.K_w):    #按下 上 或 W 鍵，改變角度
                self.request_rotation()
            elif pressed(pygame.K_LEFT) or pressed(pygame.K_a):  #按下 左 或 A鍵，往左移動
                self.request_movement('left')
                self.movement_keys['left'] = 1
            elif pressed(pygame.K_RIGHT) or pressed(pygame.K_d): #按下 右 或 D鍵，往右移動
